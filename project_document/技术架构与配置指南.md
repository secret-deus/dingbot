...
# （可选）查阅 archived/k8s-mcp-standalone 中历史独立服务的 config 示例
```
...
# K8s/ECS 工具在主进程内加载；独立 HTTP MCP 见 archived/k8s-mcp-standalone
...
主应用使用根目录 `pyproject.toml`。K8s/ECS 工具代码位于 `backend/src/k8s_mcp`、`backend/src/ecs_mcp`；原独立工程已归档至 `archived/`。SSH 等仍可为独立子模块：

```
ding-robot/
├── pyproject.toml          # 主项目配置
├── backend/                # 后端API模块（含进程内 K8s/ECS 工具）
├── archived/               # 历史独立 k8s-mcp / ecs-mcp
└── ssh-jumpserver-mcp/     # SSH 跳板机 MCP（如使用）
```
