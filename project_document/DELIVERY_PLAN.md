...
4. 产品化治理：审计筛选、真实用户管理和危险工具确认流已完成，详见
   `project_document/specs/governance-hardening/`；下一步进入发布前的全链路自测和
   真实资源 smoke 条件收集。
...
| 权限页只有静态角色和弱审计查询 | 管理员无法按资源/结果定位权限事件 | 已补 `/config/audit` admin-only、真实用户管理和危险工具确认流 |
