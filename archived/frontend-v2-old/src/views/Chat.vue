    ...
    <div class="chat-body cockpit-grid" :class="{ 'history-collapsed': !sidebarVisible }">
      ...
      <el-main class="chat-main cockpit-panel">
        ...
        <StreamChat
          ...
          :mcp-context-count="mcpRuntime.enabled"
          :mcp-total-count="mcpRuntime.total"
          :skill-id="chatSettings.skillId"
        ...
        <div class="context-body">
          ...
          <section class="context-card">
            <div class="card-title-row">
              <h4>工具总线</h4>
              <el-switch
                v-model="chatSettings.enableTools"
                size="small"
                inline-prompt
                active-text="ON"
                inactive-text="OFF"
            ...
            <div class="runbook-stack">
              ...
              <div class="runbook-item">
                <span class="status-dot cyan"></span>
                {{ mcpRuntime.enabled }}/{{ mcpRuntime.total }} servers in AI context
              </div>
              <div class="runbook-item">
                <span class="status-dot"></span>
              ...
              <MCPServerSwitches
                class="context-mcp-switches"
                :auto-load="true"
                :show-global-controls="true"
                @servers-changed="handleMcpServersChanged"
                @server-toggled="handleMcpServerToggled"
              />
          </section>
      ...
      <el-form :model="chatSettings" label-width="120px">
        <el-form-item label="启用工具调用">
          ...
          <div class="setting-help">
            控制是否把 MCP 工具带入本次对话上下文；MCP runtime 会保持常驻运行
          </div>
        </el-form-item>

        <el-form-item label="工具策略">
          <el-select v-model="chatSettings.skillId" style="width: 100%">
            <el-option label="K8s 只读" value="k8s_readonly" />
            <el-option label="运维只读" value="ops_readonly" />
            <el-option label="全量工具" value="full" />
          </el-select>
          <div class="setting-help">
            默认使用只读工具，降低误操作风险
          </div>
...
import MCPServerSwitches from '@/components/MCPServerSwitches.vue'
...
const handleMcpServersChanged = (data) => {
  const servers = Array.isArray(data.servers) ? data.servers : []
  mcpRuntime.value = {
    total: servers.length,
    enabled: data.total_enabled ?? servers.filter((server) => server.enabled).length,
    connected: data.total_connected ?? servers.filter((server) => server.connected).length,
    servers
  }
}

const handleMcpServerToggled = () => {}
...
.chat-cockpit-page {
  --cockpit-bg: var(--ops-bg);
  --cockpit-panel: var(--ops-surface);
  --cockpit-panel-2: var(--ops-surface-raised);
  --cockpit-border: var(--ops-border);
  --cockpit-border-strong: var(--ops-border-strong);
  --cockpit-text: var(--ops-text);
  --cockpit-muted: var(--ops-muted);
  --cockpit-accent: var(--ops-accent);
  --cockpit-green: var(--ops-success);
  --cockpit-amber: var(--ops-warning);
  height: 100%;
  min-height: 0;
  display: grid;
...
.command-actions :deep(.el-button:hover) {
  ...
  background: rgba(214, 168, 79, 0.11) !important;
}
...
.status-dot.cyan {
  ...
  box-shadow: 0 0 10px rgba(214, 168, 79, 0.48);
}
...
.mini-tile {
  ...
  border-radius: 8px;
  background: #171a14;
  border: 1px solid rgba(58, 61, 48, 0.86);
}
...
.context-mcp-switches {
  margin-top: 14px;
}

.context-mcp-switches :deep(.global-status),
.context-mcp-switches :deep(.mcp-tooltip) {
  border: 1px solid rgba(58, 61, 48, 0.86);
  border-radius: 8px;
  background: #151712;
  color: #a8aa98;
...
.context-mcp-switches :deep(.server-switches) {
  gap: 8px;
}

.context-mcp-switches :deep(.server-switch-item) {
  align-items: flex-start;
  flex-direction: column;
  padding: 10px;
  border-color: rgba(58, 61, 48, 0.86);
  border-radius: 8px;
  background: #10120e;
}

.context-mcp-switches :deep(.server-switch-item.enabled),
.context-mcp-switches :deep(.server-switch-item.connected) {
  border-color: rgba(143, 191, 135, 0.42);
  background: rgba(143, 191, 135, 0.08);
}

.context-mcp-switches :deep(.server-name) {
  color: #f1efe7;
}

.context-mcp-switches :deep(.server-details) {
  flex-wrap: wrap;
  gap: 6px;
  color: #9da08e;
}

.context-mcp-switches :deep(.server-controls) {
  flex-direction: row;
  justify-content: space-between;
  width: 100%;
}
...
.chat-cockpit-page :deep(.session-card-active) {
  border-color: rgba(214, 168, 79, 0.62) !important;
...
.chat-cockpit-page :deep(.el-switch.is-checked .el-switch__core) {
  border-color: #8fbf87 !important;
  background-color: #8fbf87 !important;
}

.chat-cockpit-page :deep(.el-switch__core) {
