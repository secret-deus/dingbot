...
import {
  ...
  VideoPause, VideoPlay, InfoFilled, CircleCheckFilled, CircleCloseFilled, ArrowRight
...
const toolsAllowed = computed(() => props.enableTools)
const toolsStatusText = computed(() => {
  if (!props.enableTools) return 'Tools Off'
  if (!props.mcpTotalCount) return 'MCP On'
  return `MCP On · context ${props.mcpContextCount}/${props.mcpTotalCount}`
})
...
// 重连
const reconnect = async ({ silent = false } = {}) => {
...
.question-chip:hover {
  border-color: rgba(214, 168, 79, 0.36) !important;
  background: #f1eadb !important;
  transform: translateY(-2px) scale(1.02);
  box-shadow: 0 4px 12px rgba(92, 77, 42, 0.12) !important;
}
...
.user-message-card:hover {
  ...
  box-shadow: 0 6px 20px rgba(92, 77, 42, 0.12);
  transform: translateY(-2px);
}
...
.spinner {
  width: 12px;
  height: 12px;
  border: 2px solid rgba(214, 168, 79, 0.22);
  border-top-color: var(--ops-accent);
  border-radius: 50%;
...
.stream-chat {
  --stream-bg: #090a08;
  --stream-panel: #12140f;
  --stream-panel-2: #171a14;
  --stream-border: #2a2d24;
  --stream-border-strong: #3a3d30;
  --stream-text: #f1efe7;
  --stream-muted: #9da08e;
  --stream-cyan: var(--ops-accent);
  --stream-green: #9fcf96;
  --stream-amber: #e4bd68;
  height: 100%;
  min-height: 0;
...
.status-tag-item {
  border-color: rgba(214, 168, 79, 0.3) !important;
  background: rgba(214, 168, 79, 0.08) !important;
  color: var(--stream-cyan) !important;
}
...
.tool-call-card {
  border-width: 1px !important;
  border-radius: 8px !important;
  background: rgba(8, 16, 20, 0.72) !important;
}
...
.input-card {
  max-width: 1040px !important;
  border: 1px solid rgba(214, 168, 79, 0.24) !important;
  border-bottom-color: rgba(214, 168, 79, 0.36) !important;
