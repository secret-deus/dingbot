...
export const api = {
  ...
  chat: {
    ...
    streamChat: async (message, enableTools = true, options = {}) => {
      const token = localStorage.getItem(AUTH_TOKEN_KEY)
  ...
  mcp: {
    callTool: async (toolName, parameters = {}, context = {}) => unwrapApiResponse(
      await apiClient.post(`/v2/tools/${encodeURIComponent(toolName)}/call`, {
        parameters,
        context
      })
    )
