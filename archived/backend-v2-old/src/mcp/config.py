...
class MCPServerConfig(BaseModel):
    ...
    type: str = Field(..., description="服务器类型", pattern="^(sse|stdio|http|local)$")
    enabled: bool = Field(True, description="是否启用")
    provider: Optional[Literal["k8s", "ecs"]] = Field(
        None,
        description="local 类型的本地工具提供者",
    )
    ...
    # 实现方式：builtin/local=逻辑在主进程内，不连远程；remote=独立 MCP 进程
    implementation: Optional[Literal["builtin", "remote"]] = Field(
        None,
        description="builtin 表示由主应用进程内注册，客户端应跳过远程连接该 name",
    )

    @field_validator('type')
    @classmethod
    def validate_type(cls, v):
        valid_types = ['sse', 'stdio', 'local']
