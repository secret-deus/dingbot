...
class EnhancedMCPClient:
    ...
    def _connectable_remote_servers(self):
        """需要常驻连接的远程 MCP（避免与 builtin K8s/ECS 重复）。"""
        config = self.config_manager.get_config()
        servers = list(getattr(config, "servers", []) or [])
        servers = [s for s in servers if getattr(s, "type", None) != "local"]
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            remote_servers = self._connectable_remote_servers()
            connection_tasks = []
            ...
            if len(self.tools) > 0:
                self.status = MCPConnectionStatus.CONNECTED
                logger.info(
                    f"MCP 就绪: 远程常驻连接 {connected_count}/{len(remote_servers)} 个服务器, "
    ...
    def _collect_tools(self, warn_if_empty: bool = True):
        ...
        if len(self.tools) == 0 and warn_if_empty:
            ...
            logger.warning(f"   2. 服务器是否已纳入 AI 上下文")
