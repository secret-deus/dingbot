...
async def batch_toggle_servers(
    ...
    try:
        ...
        config_manager.save_config()
        if mcp_client and hasattr(mcp_client, "_refresh_tools"):
            mcp_client._refresh_tools()
...
async def toggle_mcp_server(
    ...
    """切换 MCP 服务器是否纳入 AI 上下文/工具可调用范围。

    MCP runtime 的连接生命周期由服务端常驻管理；这里仅更新工具准入范围。
    """
    try:
        server = config_manager.get_server_by_name(server_name)
        if not server:
        ...
        status = "纳入 AI 上下文" if server.enabled else "从 AI 上下文排除"
